...
if (nuevavisita) { 
            ...
            ck.setMaxAge(30);   //para poner un comentario 
ck.setComment("Control de nuevos visitantes"); 
            //poner la cookie en el navegador           
            response.addCookie(ck);  mensaje="Gracias por visitar la pagina"; 
            visita="Nro de Visitas: 1"; 
        }else{ 
           int cont = Integer.parseInt(valor); 
           cont++; 
            Cookie ck = new Cookie("visitante",""+cont);             
            ck.setMaxAge(30);            response.addCookie(ck); 
            mensaje="Estamos agradecidos por su visita"; 
